...
package com.mdground.guest.view.wheelview;
...
import com.mdground.guest.api.utils.PxUtil;
import com.mdground.guest.view.wheelview.adapters.AbstractWheelTextAdapter;
import com.mdground.guest.view.wheelview.adapters.WheelViewAdapter;
...
public class WheelView extends View {
	...
	private int text_highlight_color_resId = R.color.color_000000;

	private int text_normal_color_resId = R.color.color_c3c3c3;
	...
	private int wheelForeground = R.color.color_00000000;
	...
	private void LogOnMeasureInfo(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// KLog.json(this, widthSpecModelString);
		// Log.e"TAG, heightSpecModeString);
		// KLog.json(this, widthSpecSize + "px");
		// KLog.json(this, heightSpecSize + "px");
		// KLog.json(this, widthSpecSize2Dp + "dp");
		// KLog.json(this, heightSpecSize2Dp + "dp");
	}
	...
	private void drawCenterRect(Canvas canvas) {
		...
		// KLog.json(this, (getItemHeight() / 2) + "");
	...
	private boolean rebuildItems() {
		// KLog.json(this, "rebuildItems被调用了");
	...
	private void buildViewForMeasuring() { // 初始化一次,起码调用四次以上,因为onMeasure()被调用四次以上
		// KLog.json("buildViewForMeasuring被调用了");
		// clear all items
		if (itemsLayout != null) {
			recycle.recycleItems(itemsLayout, firstItem, new ItemsRange());
		} else {
	...
	private boolean addViewItem(int index, boolean first) {
		...
		// KLog.json("是否是Identity : " + view.getMatrix().isIdentity());
	...
	private View getItemView(int index) {
		// KLog.json(this, "getItemView被调用了");
		if (viewAdapter == null || viewAdapter.getItemsCount() == 0) {
			return null;
		}
